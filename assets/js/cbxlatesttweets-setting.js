(function ($) {
	...
	$(document).ready(function () {
		...
		$('.selecttwo-select').select2({
			placeholder: cbxlatesttweets_setting.please_select,
			allowClear: false
		});
		...
		if (typeof (localStorage) !== 'undefined') {
			activetab = localStorage.getItem('cbxlatesttweetsloactivetab');
		}
		...
		if (window.location.hash) {
			if ($(window.location.hash).hasClass('global_setting_group')) {
				...
				if (typeof (localStorage) !== 'undefined') {
					localStorage.setItem('cbxlatesttweetsloactivetab', activetab);
				}
			}
		}
		...
		$('.nav-tab-wrapper a').on('click', function (e) {
			...
			if (typeof (localStorage) !== 'undefined') {
				localStorage.setItem('cbxlatesttweetsloactivetab', $(this).attr('href'));
			}
			$('.global_setting_group').hide();
		...
		$('#cbxlatesttweets_tools').on('click', '.cbxlatesttweets_jump', function (e) {
			e.preventDefault();
		...
		$('#cbxlatesttweets_info_trig').on('click', function (e) {
			e.preventDefault();

			$('#cbxlatesttweets_resetinfo').toggle();
		});
		...
		$('#save_settings').on('click', function (e) {
			...
			$('#' + $tab_id).find('.submit_cbxlatesttweets').trigger('click');
		});
	});
