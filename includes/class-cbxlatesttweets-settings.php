 ...
 */
if ( ! class_exists( 'CBXLatestTweets_Settings_API' ) ):
	class CBXLatestTweets_Settings_API {
		...
		function callback_text( $args ) {
			...
			$html = sprintf( '<input type="%1$s" class="%2$s-text" id="%3$s[%4$s]" name="%3$s[%4$s]" value="%5$s" %6$s %7$s %8$s/>', $type, $size, $args['section'], $args['id'], $value, $required, $minlength_html, $maxlength_html );
		...
		function callback_number( $args ) {
			...
			$html = sprintf( '<input type="%1$s" class="%2$s-text" id="%3$s[%4$s]" name="%3$s[%4$s]" value="%5$s"%6$s %7$s %8$s %9$s %10$s/>', $type, $size, $args['section'], $args['id'], $value, $placeholder, $min, $max, $step, $required );
			$html .= $this->get_field_description( $args );
			echo $html;
		}
		...
		function callback_checkbox( $args ) {
			...
			$html         .= sprintf( '<input type="checkbox" class="checkbox" id="wpuf-%1$s[%2$s]" name="%1$s[%2$s]" value="on" %3$s %4$s />', $args['section'], $args['id'], checked( $value, 1, false ), $required );
        ...
        function callback_heading( $args ) {
            ...
            $html = '<h3 class="setting_heading"><span class="setting_heading_title">' . $args['name'] . '</span><a title="' . esc_attr__( 'Click to show hide', 'cbxlatesttweets' ) . '" class="setting_heading_toggle" href="#">' . esc_attr__( 'Click to show hide', 'cbxlatesttweets' ) . '</a></h3>';
            $html .= $this->get_field_description( $args );
		 ...
		 * @param array $args settings field args
		 */
		function callback_redirect_uri_pattern( $args ) {
			$value = esc_attr( $this->get_option( $args['id'], $args['section'], $args['default'] ) );

			$html = sprintf( '<label for="wpuf-%1$s[%2$s]"><code class="cbxlatesttweetsclipboard">', $args['section'], $args['id'] );
			//                $html .= sprintf('%1$s</code></label><br>', $value);
			$html .= sprintf( '%1$s</code><span class="cbxlatesttweetsclipboardtrigger" data-clipboard-target=".cbxlatesttweetsclipboard" title="' . esc_html__( "Copy to clipboard", "cbxlatesttweets" ) . '">
                            <img style="width: 16px; height: 16px;" src="' . plugins_url( 'includes/assets/img/clippy_18.png', dirname( __FILE__ ) ) . '" alt="' . esc_html__( 'Copy to clipboard', 'cbxlatesttweets' ) . '">
            ...
            <div class="metabox-holder">
                ...
                foreach ( $this->settings_sections as $form ) {
                    ...
                    <div id="<?php echo $form['id']; ?>" class="global_setting_group" style="<?php echo $display_style; ?>">
                        <form method="post" action="options.php" class="cbxlatesttweets_setting_form">
							<?php
							do_action( 'cbxlatesttweets_setting_top_' . $form['id'], $form );
							settings_fields( $form['id'] );
							do_settings_sections( $form['id'] );
							do_action( 'cbxlatesttweets_setting_bottom_' . $form['id'], $form );
							?>
                            <div style="padding-left: 10px">
		                        <?php submit_button(esc_html__('Save Settings', 'cbxlatesttweets'), 'primary submit_cbxlatesttweets', 'submit', true, array('id' => 'submit_'.esc_attr($form['id']))); ?>
                            </div>
                        </form>
                    </div>
                    <?php
